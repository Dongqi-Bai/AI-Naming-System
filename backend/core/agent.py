...
from env_util import DEEPSEEK_API_KEY
...
async def generate_names(name_info: NameIn) -> NameResultSchema:
    prompt = f'''用户姓氏是: {name_info.surname}，性别是: {name_info.gender}，名字字数要求是:
    {name_info.length}，其他要求是:
    {name_info.other}，这些名字不要：{'、'.join(name_info.exclude)}'''
